...
folder_train='../resave_kagle/train';
folder_valid='../resave_kagle/valid';
...
inDs = imageDatastore([folder_train '/data']  ,'FileExtensions','.png');
outDs = imageDatastore([folder_train '/gt'] ,'FileExtensions','.png','ReadFcn',@customreaderOut);
...
inDs = imageDatastore([folder_train '/data'],'FileExtensions','.png','ReadFcn',@customreaderIn);
if classif
    outDs = pixelLabelDatastore([folder_train '/gt'],classNames,pixelLabelIDs,'FileExtensions','.png','ReadFcn',@customreaderOutClassif);    
else
    outDs = imageDatastore([folder_train '/gt'],'FileExtensions','.png','ReadFcn',@customreaderOut);
end
...
if length(folder_valid)>0
    inDs_v = imageDatastore([folder_valid '/data'],'FileExtensions','.png','ReadFcn',@customreaderIn);
    if classif
        outDs_v = pixelLabelDatastore([folder_valid '/gt'],classNames,pixelLabelIDs,'FileExtensions','.png','ReadFcn',@customreaderOutClassif);    
    else
        outDs_v = imageDatastore([folder_valid '/gt'],'FileExtensions','.png','ReadFcn',@customreaderOut);
    end
